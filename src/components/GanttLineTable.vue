...
const init = () => {
  ...
  props.source.forEach((item) => {
    ...
    item.detailList.forEach((detailItem: any, index: number) => {
      if (detailItem.nodeId.includes("__LineTable.node.ft")) {
        height += 20;
        translateY += 20;
      }
      let child = {
        strokeColor: props.statusColor[item.shipStatus],
        projectDesc: index == 0 ? item.projectDesc : "",
